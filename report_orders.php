...
<?php
	...
	if(!empty($_POST["search"]["post_at"])) {			
		...
		$queryCondition .= "WHERE inventory_order.inventory_order_created_date BETWEEN '$fiy-$fim-$fid' AND '" . $post_at_todate . "'";
	}

	$sql = "SELECT * from inventory_order_product 
	inner join inventory_order 
	on inventory_order_product.inventory_order_id = inventory_order.inventory_order_id
	inner join stock 
	on inventory_order_product.stock_id = stock.stock_id " . $queryCondition . " ORDER BY inventory_order_product.inventory_order_product_id asc";
...
<table class="table-content">
	...
	?>
        <tr>
			...
			<td><?php echo $row["deduct"] .$row["uom"] ?></td>
			<td><?php echo $row["qty_nett"]; ?></td>
			<td><?php echo $row["price"]; ?></td>
			<td><?php echo 'R' .$row["nett_price"]; ?></td>
