...
?>
	...
	<script>
		...
		function stockcheck(stock,orderqty) {
		
		//stock check
		if (!stock.stock_qty || stock.stock_qty < inventory_order_product.quantity) {
            var msg = !stock.stock_qty ? 'Please note that product ' + stock.item_name + ' is out of stock at the moment, so it will go on Back Order!' : 'Please note that current stock on hand for product ' + stock.item_name + ' is ' + stock.stock_qty + ', so the rest will go on Back Order!';
            $alert({ content: msg, duration: 5, placement: 'top-right', type: 'warning', show: true});
          }
		}
...
<script type="text/javascript">
    $(document).ready(function(){
		...
		function add_product_row(count = '')
		{
			...
			html += '</div>';
			html += '<div class="col-md-2">';
			html += '<input type="text" onclick="stockcheck()" class="form-control" id="quantity'+count+'" name="quantity[]" placeholder="Gross" onfocus="this.oldvalue = this.value;" onkeyup="validateInput(this);this.oldvalue = this.value;" onchange="validateInput(this);this.oldvalue = this.value;" class="form-control"  required />';
			html += '</div>';
			html += '<div class="col-md-2">';
			...
			html += '<input type="hidden" id="item_date'+count+'" name="item_date[]" class="form-control" value="<?php echo date('m/d/y');?>"  />';
			html += '</div>';
			html += '<div class="col-md-1">';
			if(count == '')
			{
